...
const ManageOffer = ({
  ...
  const rejectDemand = (e)=>{
    ...
    axios.post(
      `/offer/${_id}/demand/sort`,
      JSON.stringify({_id: userId, status: 'rejected'})
    ).then((res)=>{
      setCollaborationDemands(
        collaborationDemands.filter((demand)=>String(demand.from._id)!==String(res.data.from)));
  ...
  const acceptDemand = (e)=>{
    ...
    axios.post(
      `/offer/${_id}/demand/sort`,
      JSON.stringify({_id: userId, status: 'accepted'})
    ).then((res)=>{
      ...
      setDisplayedCollaborators([...displayedCollaborators, {
        _id: userId, firstName: from.firstName, lastName: from.lastName
      }]);
      setCollaborationDemands(
        collaborationDemands.filter((demand)=>String(demand.from._id)!==String(res.data.from))
      );
          ...
          {displayedCollaborators.map((collaborator)=>
            <Link
              ...
              <img
                ...
                src={process.env.DEV_URL+'/users/'+collaborator._id+'/avatar'}/>
      ...
      {String(user._id)===String(publisherId)
        ? <div>
                        ...
                        <div className="manager__sidebar-body">
                          ...
                          {collaborationDemands.map((demand)=>
                            <div key={demand.from._id}>
                              <Link
                                to={'/profile/'+demand.from._id}
                                className="offer-element__comment-subheader">
                                <img
                                  className="header__picture offer-element__comment-picture"
                                  src={process.env.DEV_URL+'/users/'+demand.from._id+'/avatar'}/>
                                <p>{demand.from.firstName} {demand.from.lastName}</p>
                              </Link>
                              <p>{demand.message}</p>
                              <button
                                className="manager__button manager__button-margin"
                                id={'accept'+demand.from._id}
                                onClick={acceptDemand}><AiOutlineCheckCircle/> Accepter</button>
                              <button
                                className="manager__button"
                                id={'reject'+demand.from._id}
                                onClick={rejectDemand}><TiDeleteOutline/> Rejeter</button>
                            </div>)}
                        </div>
          }
          <div className="manager__sidebar-bottom">
            <button
              className="manager__button"
              onClick={()=>{history.push('/offer/edit/'+_id);}}
            ><AiOutlineEdit/> Modifier l&apos;annonce</button>
            <button
              className="manager__button"
              onClick={deleteOffer}><MdDeleteForever/>
              Supprimer l&apos;annonce définitivement</button>
          </div>
        </div>
        :                <div className="manager__sidebar-body">
          <h3>Statut de l&apos;annonce : </h3>
