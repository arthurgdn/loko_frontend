...
const profileReducer = (state=profileReducerDefaultState,action)=>{
    ...
    switch(action.type){
        ...
        case 'EDIT_PROFILE':
            return {...state,...action.updates,editProfileError:''}
        case 'SEND_RECOMMENDATION':
            return {...state,recommendations : [...state.recommendations,action.recommendation],sendRecommendationError:'' }
        case 'ADD_COMPLETED_OFFER':
            return {...state,completedOffers : [...completedOffers,action.completedOffer],addCompletedOfferError:''}
