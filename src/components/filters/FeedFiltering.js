...
import {
  sortByPoints, sortByDate, setDistanceRadius, setTextFilter
} from '../../actions/feedFilters';

const createSliderWithTooltip = Slider.createSliderWithTooltip;
const TippedSlider = createSliderWithTooltip(Slider);

const FeedFiltering = ({
  filters, setDistanceRadius, setTextFilter
})=>{
  const [text, setText] = useState(filters.text);
  const [displayIcon, setDisplayIcon] = useState(true);
  const [distanceRadius, setStateDistanceRadius] = useState(
    Math.round(1000*Math.log10(10*filters.distanceRadius))
  );
...
const mapStateToProps = (state)=>({filters: state.feedFilters});
