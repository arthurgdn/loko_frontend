...
const EditUserForm = ({
  user, startEditUserInfo, editUserInfoError
})=>{
  ...
  const [location, setLocation] = useState(
    user.location?user.location:{type: 'Point', coordinates: []});
    ...
    if(useBrowser) {
      if(navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position)=>{
          ...
          access_token=${process.env.MAPBOX_API_KEY}&language=fr&limit=1`).then(
            (res)=>{
          ...
          console.log(error);
          setError(
            'Votre appareil ne supporte pas la geolocalisation,' +
            'veuillez entrer votre position manuellement');
        });
      }else{
  ...
  const onLocationInputChange = (e)=>{
    ...
    axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/
    ${encodeURIComponent(locationInput)}.json?
    access_token=${process.env.MAPBOX_API_KEY}&language=fr&limit=1`).then(
      (res)=>{
  ...
  return (
    <div>
      ...
      <form onSubmit={onSubmit} className="settings__form">
        ...
        {locationText&& <p>Votre position s&apos;affiche :
...
const mapDispatchToProps = (dispatch)=>({
  startEditUserInfo: (updates, profilePicture)=>
    dispatch(startEditUserInfo(updates, profilePicture)),
