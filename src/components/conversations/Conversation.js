...
import {
  startSetConversation, startPatchMembers, startAddAdmin
...
const Conversation =  ({
  ...
  const isAdmin = Object.keys(conversation).length>2 &&
    !!conversation.admins.find((admin)=>admin.admin===user_id);
  ...
  useEffect(()=>{
    ...
    if(collaborators.length>0 &&
        !!stateConversation.members &&
        Object.keys(collaborators[0]).length===3||collaborators.length===0) {
  ...
  return (
    <div>
      ...
      {Object.keys(conversation).length<=2
        ? !frontSetConvError && !editConvError&&
        <p className="group__text-info">Aucune conversation ne correspond</p>
            ...
            <div className="group__sidebar">
              <div className="manager__sidebar-container">
                        ...
                        {conversation.members.map((member)=>
                          ...
                          >
                            <img
                              ...
                              src={process.env.DEV_URL+'/users/'+member.member+'/avatar'}
                            />
                            <p>
                              {!!conversation.admins
                                .find((admin)=>admin.admin===member.member) && <RiAdminLine/>}
                ...
                <div>
                  {isAdmin &&
                    conversation.members.length>2 &&
                    conversation.admins.length!==conversation.members.length &&
                    <div className="manager__sidebar-body">
                      <h3>Gérer les membres : </h3>
                      {conversation.members.filter((member)=>{
                        return member.member !== user_id &&
                        !conversation.admins.find((admin)=>admin.admin===member.member);
                      }).map((member)=>
                        <div key={member.member}>
                          ...
                          <button
                            ...
                            id={'admin'+member.member}
                            onClick={addAdmin}><GiUpgrade/> Promouvoir</button>
                          <button
                ...
                <div className="manager__sidebar-body">
                  <button
                    className="manager__button"
                    onClick={()=>setDisplayEditConvInfoForm(!displayEditConvInfoForm)}>
                    <AiOutlineEdit/> Modifier</button>
                  {isAdmin &&
                    displayEditConvInfoForm&&
                    <EditConversationInfoForm
                      {...conversation}
                      setDisplayEditConvInfoForm={setDisplayEditConvInfoForm}/>}
