...
export default ({
  author, content, createdAt, user_id
})=>
  <div
    className={ author._id ===user_id
      ? 'conversation__parent-message-user'
      :'conversation__parent-message-other'}>
    <div className="conversation__message">
      <div className="conversation__message-header">
        ...
        <Link to={'/profile/'+author._id} className="conversation__message-subheader">
          ...
          <img
            ...
            src={`${process.env.DEV_URL}/users/${author._id}/avatar`}/>
          {author._id!==user_id && <p>{author.firstName} {author.lastName}</p>}
        </Link>
      ...
      <p
        className="conversation__message-content" dangerouslySetInnerHTML={{ __html: content }}></p>
