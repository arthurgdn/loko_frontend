...
const userReducer = (state=userReducerDefaultState, action)=>{
  ...
  case 'EDIT_USER_INFO':
    return {
      ...state, ...action.updates, editUserInfoError: ''
    };
  case 'NEW_COLLAB':
    return {
      ...state, collaborators: [...state.collaborators, action.collaborator], newCollabError: ''
    };
  case 'SET_COLLABORATION_DEMANDS':
    return {
      ...state, collaborationDemands: action.collaborationDemands, setCollaborationDemandsError: ''
    };
  case 'SET_COLLABORATORS':
    return {
      ...state, collaborators: action.collaborators, setCollaboratorsError: ''
    };
  case 'ADD_USER_KEYWORD':
    return {...state, userKeywords: [...state.userKeywords, action.keyword]};
  case 'ADD_PHONE':
    return {
      ...state, phoneNumber: action.phoneNumber, error: ''
    };
  case 'USER_GROUP_CREATED' :
    return {
      ...state, userGroups: [...state.userGroups, action.group], userGroupCreatedError: ''
    };
  case 'REMOVE_COLLAB_DEMAND':
    return {
      ...state, error: '',
      collaborationDemands: state.collaborationDemands.filter(
        (demand)=>demand.demand!==action.collaborator)
    };
  case 'SET_USER_ERROR':
