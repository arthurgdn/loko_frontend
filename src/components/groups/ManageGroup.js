...
const ManageGroup = ({
  collaborators, startSetCollaborators, group, history
}) =>{
    ...
    async function getMembers () {
      try{
        if(group.membership==='admin' && group.securityStatus ==='onRequest') {
          const requestsResponse = await axios.get('/group/'+group._id+'/members?status=requested');
    ...
    if(collaborators.length>0 &&
      ...
      for (const collaborator of collaborators) {
        ...
        if (!members.find((member)=>
          member.user===collaborator.collaborator) &&
        !admins.find((admin)=>admin.user===collaborator.collaborator)) {
  ...
  const acceptRequest = (e)=>{
    ...
    axios.patch(
      '/group/'+group._id+'/member',
      JSON.stringify({updates: {status: 'member'}, _id: userId}))
      .then((res)=>{
  ...
  const removeMember = (e)=>{
    const userId = e.currentTarget.id.substr(6, e.currentTarget.id.length);
    axios.post('/group/'+group._id+'/member/delete', JSON.stringify({_id: userId}))
      .then(()=>{setMembers(members.filter((member)=>member.user!==userId));})
      .catch((e)=>{
        console.log(e);
        setError('Erreur serveur');});
  };
  const promoteAdmin = (e)=>{
    ...
    axios.patch(
      '/group/'+group._id+'/member',
      JSON.stringify({updates: {status: 'admin'}, _id: userId})
    )
      .then((res)=>{
          ...
          {admins.map((admin)=>
            ...
            >
              <img
                ...
                src={process.env.DEV_URL+'/users/'+admin.user+'/avatar'}/>
                    ...
                    {members.map((member)=>
                      <Link
                        ...
                        <img
                          ...
                          src={process.env.DEV_URL+'/users/'+member.user+'/avatar'}/>
                ...
                <div>
                        ...
                        <div className="manager__sidebar-body">
                          ...
                          {membershipRequests.map((memberRequest)=>
                            <div key={memberRequest.user}>
                              <Link
                                to={'/profile/'+memberRequest.user}
                                className="offer-element__comment-subheader"
                              >
                                <img
                                  className="header__picture offer-element__comment-picture"
                                  src={process.env.DEV_URL+'/users/'+memberRequest.user+'/avatar'}/>
                                <p>{memberRequest.firstName} {memberRequest.lastName}</p>
                              </Link>
                              <button
                                className="manager__button manager__button-margin"
                                id={'accept'+memberRequest.user}
                                onClick={acceptRequest}><AiOutlineCheckCircle/> Accepter</button>
                              <button
                                className="manager__button"
                                id={'reject'+memberRequest.user}
                                onClick={rejectRequest}><TiDeleteOutline/> Rejeter</button>
                            </div>
                          )}
                        </div>
                  }
                  ...
                  {group.securityStatus ==='private' &&
                        <div className="manager__sidebar-body">
                          ...
                          <button
                            className="manager__button"
                            onClick={inviteMember}>Inviter</button>
                        </div>
                  }
                  <div className="manager__sidebar-body">
                    <button
                      className="manager__button"
                      onClick={()=>history.push('/group/edit/'+group._id)}
                    ><AiOutlineEdit/> Modifier</button>
                  </div>
                    ...
                    <div className="manager__sidebar-body">
                      ...
                      {members.map((member)=>
                        <div key={member.user}>
                          ...
                          <button
                            className="manager__button manager__button-margin"
                            id={'promote'+member.user}
                            onClick={promoteAdmin}><GiUpgrade/> Promouvoir</button>
                          <button
                            className="manager__button"
                            id={'remove'+member.user}
                            onClick={removeMember}><TiDeleteOutline/> Supprimer</button>
