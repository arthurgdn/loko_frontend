...
import  {
  AiOutlineEdit, AiOutlineUserAdd, AiOutlineMessage, AiOutlineCheckCircle
} from 'react-icons/ai';
...
const ProfilePage = ({
  startSendCollaboration,
  startSetProfile,
  startSendRecommendation,
  match,
  stateProfile,
  user_id,
  collaborators,
  ...
  const isCollaborator = collaborators.find(
    (collaborator)=>String(collaborator.collaborator)===match.params.id);
  const isInCollaborationDemands = collaborationDemands.find(

    (demand)=>String(demand.demand)===match.params.id);
      ...
      <div className="profile__container">
        <div className="profile__header">
          <img
            ...
            src={process.env.DEV_URL+'/users/'+match.params.id+'/avatar'}/>
          <div className="profile__subheader">
            <h3>
              {profile.firstName} {profile.lastName} {isCollaborator && <FiUserCheck/>}
              {demandSent && <FaHourglassEnd/>}</h3>

            {match.params.id!==user_id ?<div>

              {!isCollaborator &&
                !isInCollaborationDemands &&
                !demandSent && <button className="profile__button" onClick={()=>{
              ...
              <button
                ...
                <AiOutlineMessage/> Message</button></div>
              : <button
                className="profile__button"
                onClick={()=>{history.push('/settings');}}><AiOutlineEdit/> Modifier</button>}
            <div className="profile__button-align">
              {isInCollaborationDemands &&
                <button
                  className="profile__button profile__no-space"
                  id={'accept'+match.params.id} onClick={accept}><AiOutlineCheckCircle/> </button>
              }
              {isInCollaborationDemands &&
                <button
                  className="profile__button profile__no-space"
                  id={'reject'+match.params.id} onClick={reject}><TiDeleteOutline/></button>
              }
            </div>
          </div>
        </div>
        <div className="profile__body">
          ...
          {profile.locationText &&
            <p className="profile__location"><GoLocation/> {profile.locationText}</p>}
        ...
        <div className="profile__body profile__block">
          ...
          {profile.summary && profile.summary.length>0
            ?<p>{profile.summary}</p>
            :<p>L&apos;utilisateur n&apos;a renseigné encore aucune
            information sur ce qu&apos;il recherche sur Loko</p>
          }
          ...
          {profile.skills.length>0
            ? <div className="profile__horizontal-list">
              {profile.skills.map((skill)=><p
                className={profile.skills.findIndex(
                  (skillSearch)=>skillSearch===skill)===profile.skills.length -1
                  ? 'profile__last-list-item':'profile__list-item'}
                key={skill}>
                {skill}
              </p>)}
            </div>
            :<p>
            L&apos;utilisateur n&apos;a renseigné encore aucune information sur ses compétences</p>
          }
        ...
        {profile.completedOffers.length>0 &&
                    <div className="profile__body profile__block">
                      ...
                      {profile.completedOffers.map(
                        (offer)=><MiniatureOfferElement
                          key={offer.completedOffer}
                          privateLink={false} {...{...offer, _id: offer.completedOffer}} />)}
        ...
        {profile.recommendations.length>0 &&
                    <div className="offer-element__comment-section profile__body profile__block ">
                      ...
                      <h3>{profile.recommendations.length}
                      recommandation{profile.recommendations.length>1 && 's'}
                      reçue{profile.recommendations.length>1 && 's'}</h3>
                      {profile.recommendations.map((recommendation)=>
                        <div key={recommendation._id} className="offer-element__comment-display">
                          <div className="offer-element__comment-header">
                            <Link
                              to={'/profile/'+recommendation.publisher._id}
                              ...
                              <img
                                ...
                                ${recommendation.publisher._id}/avatar`}
                              />
                              <p>{recommendation.publisher.firstName}
                                {recommendation.publisher.lastName}</p>
                            </Link>
                            <span className="show-for-desktop">
                              {moment(recommendation.createdAt).lang('fr').fromNow()}</span>
                          </div>
                          <p
                            className="offer-element__comment-content"
                            dangerouslySetInnerHTML={{ __html: recommendation.content }}></p>
                        </div>)}
                    </div>
        }
...
const mapDispatchToProps = (dispatch)=>({
  ...
  startSendRecommendation: (profile_id, content)=>
    dispatch(startSendRecommendation(profile_id, content))
});
